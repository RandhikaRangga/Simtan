...
class LaporanPanen extends CI_Controller {
    ...
    // ================================ Fungsi Convert Excel =====================================
        public function convert_excel() {
            ...
            $komoditas_list = $this->Panen_model->get_komoditas();
    
            // Ambil data dari model
            $data = $this->Panen_model->get_data_panen($bulan, $tahun, $kecamatan, $desa);
    
            // Olah data panen berdasarkan komoditas
            $data_panen = [];
            foreach ($data as $item) {
                $data_panen[$item->komoditas][$item->tanggal] = $item->luas_panen;
            }
            ...
            foreach ($komoditas_list as $komoditas_item) {
                ...
                $total = 0;
                for ($i = 1; $i <= $jumlah_hari; $i++) {
                    $luas = isset($data_panen[$komoditas][$i]) ? $data_panen[$komoditas][$i] : 0;
                    $col = Coordinate::stringFromColumnIndex($i + 2);
            ...
            $filename = 'Data_panen_' . date('Ymd') . '.xlsx';
