...
class LaporanProduksi extends CI_Controller {
    ...
    public function view_petugaskantor() {
        ...
        $this->load->view('petugas_kantor/laporan_produksi', $data);
    ...
    public function convert_excel() {
        ...
        $komoditas_list = $this->Produksi_model->get_komoditas();

        // Ambil data dari model
        $data = $this->Produksi_model->get_data_produksi($bulan, $tahun, $kecamatan, $desa);

        // Olah data produksi berdasarkan komoditas
        $data_produksi = [];
        foreach ($data as $item) {
            $data_produksi[$item->komoditas][$item->tanggal] = $item->berat_produksi;
        }
        ...
        foreach ($komoditas_list as $komoditas_item) {
            ...
            $total = 0;
            for ($i = 1; $i <= $jumlah_hari; $i++) {
                $berat = isset($data_produksi[$komoditas][$i]) ? $data_produksi[$komoditas][$i] : 0;
                $col = Coordinate::stringFromColumnIndex($i + 2);
                $sheet->setCellValue($col . $row, $berat);
                $total += $berat;
            }
        ...
        $filename = 'Data_produksi_' . date('Ymd') . '.xlsx';
    ...
    private function get_nama_kecamatan($kecamatan_id) {
        $kecamatan = $this->Produksi_model->get_kecamatan_by_id($kecamatan_id);
        return $kecamatan ? $kecamatan->kecamatan : '';
    }

    // Fungsi untuk mengambil nama desa
    private function get_nama_desa($desa_id) {
        $desa = $this->Produksi_model->get_desa_by_id($desa_id);
